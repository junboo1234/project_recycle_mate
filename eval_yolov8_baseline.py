...
CLASS_NAMES = ["금속캔", "종이", "페트병", "플라스틱", "스티로폼", "비닐", "유리병", "건전지", "형광등"]
NC = len(CLASS_NAMES)
    ...
    for cls_id in range(nc):
        ...
        for preds, targets in zip(all_preds, all_targets):
            ...
            if len(cls_preds) == 0 or len(gt_boxes) == 0:
                ...
                all_conf.extend(cls_preds[:, 4].cpu().tolist() if len(cls_preds) > 0 else [])
                continue
            ...
            for pred in cls_preds:
                ...
                best_iou, best_idx = ious.max(0) if ious.dim() == 1 else (ious.max(), ious.argmax())
...
def main():
    ...
    with torch.no_grad():
        for i, batch in enumerate(test_loader):
            ...
            for j in range(images.shape[0]):
                ...
                all_preds.append(detections[j] if j < len(detections) else torch.zeros((0, 6), device=DEVICE))

            if (i + 1) % 50 == 0:
                print(f"   처리 중... {i+1}/{len(test_loader)} 배치")

    ap_list, prec_list, rec_list = evaluate_map(all_preds, all_targets, NC, iou_threshold=0.5)
    mAP50 = np.mean(ap_list)

    print(f"\n{'클래스':<12} {'AP@0.5':>8} {'Precision':>10} {'Recall':>8}")
    print("-" * 42)
    for i, name in enumerate(CLASS_NAMES):
        print(f"{name:<12} {ap_list[i]:>8.4f} {prec_list[i]:>10.4f} {rec_list[i]:>8.4f}")
    print("-" * 42)
    ...
    print(f"   Inference + NMS 평균 Latency: {avg_latency:.2f} +/- {std_latency:.2f} ms")
    print(f"   FPS: {fps:.1f}")
