...
"""
1.사전학습(Pre-trained) 배제 및 From Scratch 학습 설계
적용 내용: MODEL_NAME = "yolov8n.yaml" 설정 및 EPOCHS = 300으로 상향.

근거: 외부 데이터로 학습된 가중치에 의존하지 않고, 
직접 수집한 재활용 데이터셋만을 사용하여 모델의 
순수 성능을 검증하기 위해 백지상태(Random Initialization)에서 
시작하므로 초기 수렴에 시간이 걸리는 점을 고려하여 에폭을 300으로 넉넉히 설정.

2. 재활용품 도메인 맞춤형 Loss 가중치 조정 & Focal Loss 도입
적용 내용: box: 5.0, cls: 2.0, fl_gamma: 1.5
근거: * cls(분류) 가중치 증가: 페트병, 비닐, 유리병 
등은 투명하거나 반사되는 재질 특성상 모델이 객체의 위치(box)는
잘 찾지만 클래스를 헷갈리는 경향이 있었음. 이에 분류 오차에 대한
페널티를 기존(0.5) 대비 4배(2.0) 올려 재질 간 미세한 차이를 강제 학습.

Focal Loss (fl_gamma): 캔, 종이처럼 특징이 뚜렷해 이미 잘 맞추는 
'쉬운 샘플'이 Loss를 지배하는 현상을 막고, 헷갈리기 쉬운 '어려운 샘플'에
모델의 집중력을 할당하는 고급 손실 함수 기법을 적용.

3.과적합 방지
적용 내용: mosaic: 1.0, mixup: 0.1, hsv_h/s/v 파라미터 적용.

근거:데이터 부족 및 과적합(Overfitting)을 방지하기 위한 정통 컴퓨터 비전 기법사용.
-Mosaic/Mixup: 여러 객체가 겹쳐있거나 뭉쳐서 버려지는 실제 쓰레기장의
복잡한 환경을 시뮬레이션하여 모델이 배경이 아닌 '객체 본연의 형태'를 보게 만듦.
-HSV 변환: 실내/야외, 흐린 날 등 조명 환경에 따라 크게 변하는 플라스틱/비닐의
색상 반사에 모델이 강건해지도록(Robust) 훈련.

4. 안정성과 최적 성능을 위한 SGD + Cosine Annealing 스케줄러
적용 내용: AdamW 대신 SGD + Momentum 사용, Linear Warmup(3 Epoch) + Cosine Annealing 적용.

근거: * AdamW는 초기 수렴 속도는 빠르지만, 100 Epoch 이상의 긴 From Scratch 훈련에서는 SGD가 더
나은 일반화(Generalization) 성능과 최종 mAP를 보장.
-초기 가중치가 무작위인 상태에서 시작부터 높은 학습률(1e-2)을 주면 Loss가 폭발할 수 있기 때문에
이를 막기 위해 처음 3 Epoch 동안 학습률을 서서히 올리는 Warmup을 구현, 이후 Cosine 곡선을 따라 
부드럽게 감소시켜 안정적으로 최적점(Global Minima)에 안착하도록 설계.

5. AMP (자동 혼합 정밀도)를 통한 훈련 속도 및 자원 최적화
적용 내용: torch.amp.GradScaler 및 torch.autocast(float16) 적용.

근거: 모델 훈련 시 VRAM 사용량을 절반으로 줄이고, 최신 GPU의 Tensor Core를
활용해 연산 속도를 높임. float16 연산 시 발생할 수 있는 Gradient Underflow(기울기 소실)
현상은 GradScaler를 통해 오차 스케일을 복원함으로써, 정확도 손실 없이 학습 시간을 대폭 단축.

6. Validation 중 BatchNorm 통계치 오염 방지 (Data Leakage 차단)
적용 내용: Validation 루프 내 track_running_stats = False 처리.
근거: PyTorch에서 model.eval() 상태라 할지라도 YOLO 아키텍처 특성상 
내부적으로 Validation 데이터를 통과시킬 때 BatchNorm 레이어의 평균/분산(Running Mean/Var)이
갱신되는 치명적인 버그가 발생가능. 이를 코드로 완벽히 차단하여 검증 데이터가
학습 파라미터에 조금이라도 영향을 미치는 현상(Data Leakage)을 차단.
"""
import os
import time
import torch
...
# [최적화] pre-trained아닌 모델 쓰기위해
MODEL_NAME = "yolov8n.yaml"  # nano 모델 (베이스라인)
# [최적화]깡통모델이라 학습횟수 증가
EPOCHS = 300
...
def main():
    ...
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    # 3.5 AMP (Automatic Mixed Precision) 설정
    # [최적화]scalar 초기회
    # ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
    print("   AMP (혼합 정밀도) 활성화 완료")
    scaler = torch.amp.GradScaler("cuda")
    # 구버전 PyTorch의 경우 torch.cuda.amp.GradScaler()
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in train_loader:
            ...
            # [수정된 부분] autocast를 통해 Forward pass를 혼합 정밀도로 연산
            # 16비트와 32비트를 섞어써서 연산속도 증가,에포크증가로 인한 부담을 줄이기 위해
            ...
            # [수정된 부분] Scaler를 사용한 Backward 및 Optimizer 스텝
            # 언더플로우때문에 오차가 0으로 증발하지 않도록, backward()를 하기 전에 Loss 값에 아주 큰 수(예: 65536)를 곱해서(Scale) 크기를 키움
            scaler.scale(loss).backward()

            # gradient clipping (scaler 안에서 unscale 후 적용해야 함)
